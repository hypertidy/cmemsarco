...
Cloud-native access to Copernicus Marine (CMEMS) ARCO Zarr stores. No file downloads, no directory listings, no NetCDF wrangling - just URLs and GDAL (or whatever tool/s you like).
 
## Installation

```{r}
# install.packages("pak")
pak::pak("hypertidy/cmemsarco")
```

## CMEMS ARCO

Copernicus Marine Environment Monitoring Service (CMEMS) provides Analysis-Ready Cloud-Optimized (ARCO) Zarr stores for their marine datasets. These are chunked for two access patterns:
...
sla <- catalog |>
 ...
 get_latest_versions()  # latest version per dataset
