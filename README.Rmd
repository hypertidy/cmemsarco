...
dsn
#> 'ZARR:"/vsicurl/https://s3.waw3-1.cloudferro.com/mdl-arco-time-045/arco/SEALEVEL_GLO_PHY_L4_NRT_008_046/cmems_obs-sl_glo_phy-ssh_nrt_allsat-l4-duacs-0.25deg_P1D_202411/timeChunked.zarr"'
```
...
dsn_2d <- sprintf("%s:/adt:1", dsn)
r <- rast(dsn_2d)
plot(crop(r, ext(140, 160, -50, -30)))

```

For `/vsis3/` access (may be faster in some cases), use the `*_gdals3` columns with `cmems_setup()`:
